...
const WorkoutPage = (props) => {
  ...
  const getWorkouts = (num) => {
    ...
    Axios.get(
      "http://localhost:8080/api/v1/workouts/recent/user/" +
        props.userId +
        "/" +
        num,
      {
  ...
  const getWorkoutByDate = (date) => {
    setStartDate(date);
    Axios.get(
      "http://localhost:8080/api/v1/workouts/user/bydate/" + props.userId,
      {
        headers: {
          Authorization: `Bearer ${props.token}`,
        },
      ...
      setWorkoutByDate(
        <WorkoutCard workoutData={response.data[0]}></WorkoutCard>
      );
    });
  };
  ...
  return (
    <Container fluid>
        ...
        <Col>
          <nav className=" d-flex align-items-center justify-content-center navbar navbar-light bg-light">
            <DropdownButton
              ...
              title="Show Recent Workouts"
              style={{
                paddingRight: 20,
              }}
            >
              <Dropdown.ItemText>
                Select number of recent workouts to show:
              </Dropdown.ItemText>
              <Dropdown.Item
                as="button"
                onClick={() => {
      ...
      <Row className=" d-flex align-items-center justify-content-center" fluid>
        <nav className="navbar navbar-light bg-light">
          <div className="navbar-brand">Select By Date:</div>
        </nav>
      </Row>
      <Row className=" d-flex align-items-center justify-content-center" fluid>
        <DatePicker
          selected={startDate}
          onChange={(date) => getWorkoutByDate(date)}
        />
        <br />
        <Button
          style={{ marginLeft: 10 }}
          variant="primary"
          onClick={() => setWorkoutByDate([])}
        >
          {" "}
          Clear{" "}
        </Button>
